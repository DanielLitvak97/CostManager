...
const validateCost = (body) => {
    ...
    // Check that sum is a valid number.
    if (typeof sum !== 'number' ||
        !Number.isFinite(sum)) {
        return 'sum must be a number';
    }

    // A sum cannot have a negative value.
    if (sum < 0) {
        return 'The sum must be a positive number';
    }
...
const validateReportQuery = (query) => {
    ...
    // Check every required query parameter separately.
    if (id === undefined) {
